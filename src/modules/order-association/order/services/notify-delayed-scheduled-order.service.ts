...
@Injectable()
export class NotifyDelayedScheduledOrderService implements INotifyOrderMode {
    constructor(
    ...
    async send(order: Order, company: Company) {
        ...
        const message: INotificationMessage = {
            title: 'Pedido agendado não aberto!',
            body: `Há um pedido agendado (#${orderId}) aguardando em ${company.displayName}`,
        };
